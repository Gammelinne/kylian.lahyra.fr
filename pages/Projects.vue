          ...
          <Card>
            <template #content>
              <div class="my-2 text-justify">
                <p
                  v-for="(desc, index) in expandedDescription[project.id]
              ...
              <Button
                v-if="project.description.split('-').length > 1"
                class="my-2 mx-auto block rounded-full"
                variant="text"
                size="small"
                :label="
            ...
            <template #footer>
              ...
              <Button
                v-if="project.route"
                class="my-2 mx-2"
                :label="$t('Project.Route')"
                size="large"
                severity="secondary"
                icon="pi pi-plus-circle"
                @click="dialogVisibility[project.id] = true"
              />
            </template>
          </Card>
          <Dialog
            v-model:visible="dialogVisibility[project.id]"
            header="Routes"
            @hide="dialogVisibility[project.id] = false"
          >
            <DataTable
              :value="project.route ? Object.values(project.route) : []"
              class="my-2"
            >
