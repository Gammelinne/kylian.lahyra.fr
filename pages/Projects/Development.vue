...
const toggleDescription = (projectValue: number) => {
  expandedDescription.value[projectValue] = !expandedDescription.value[projectValue];
};
</script>
          ...
          <Card>
            <template #content>
              <div class="my-2 text-justify">
                <p v-for="(desc, index) in expandedDescription[project.id] ? project.description.split('-') : [project.description.split('-')[0]]"
                  :key="index">
                  {{ desc }}{{ !expandedDescription[project.id] && index === 0 ? '...' : '' }}
                </p>
              </div>
              <Button v-if="project.description.split('-').length > 1" class="my-2 mx-auto block rounded-full"
                variant="text" size="small"
                :label="expandedDescription[project.id] ? ' ' + $t('Project.SeeLess') : ' ' + $t('Project.SeeMore')"
                :icon="expandedDescription[project.id] ? 'pi pi-chevron-up' : 'pi pi-chevron-down'"
                @click="toggleDescription(project.id)" />

              <Carousel :value="project.images" class="my-12" :num-visible="1" :num-scroll="1" :circular="true"
